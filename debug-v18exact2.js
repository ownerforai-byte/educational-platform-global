...
// EXACT copy from fix-json-final-v18.js, with debugging added
    ...
    if (!inString) {
      ...
      if (ch === '"') {
        ...
        if (j >= content.length || content[j] === ',' || content[j] === ']' || content[j] === '}' || content[j] === ':') {
          ...
          console.log(`  ESCAPING quote at pos ${i}, nextChar='${content[j]}'`);
          result += '\\"';
          i++;
          changed = true;
        }
...
console.log('Running fixOnePass...');
...
for (let i = 0; i < Math.max(c.length, fixed.length); i++) {
  if (c[i] !== fixed[i]) { diffCount++; console.log(`diff[${i}]: orig='${c[i]}' fixed='${fixed[i]}'`); }
}
