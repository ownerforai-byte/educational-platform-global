    ...
    <div className="mx-auto max-w-5xl space-y-8 py-10">
      ...
      <div>
        <h1 className="text-2xl font-bold tracking-tight capitalize">
          {classSlug.split("-").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ")} — Derivations
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {entries.length} derivation{entries.length !== 1 ? "s" : ""} across {subjectMap.size} subject{subjectMap.size !== 1 ? "s" : ""}.
        </p>
      </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {subjectEntries.length} derivation{subjectEntries.length !== 1 ? "s" : ""} · {subjectData?.units.length ?? 0} units
                    </p>
                  </div>
