...
import {
  ...
  BookOpen, ListChecks, Eye, Calculator, ScrollText, ShieldAlert, ExternalLink, Layers,
} from "lucide-react";
import { getPracticalBySlug, totalExperiments } from "@/lib/practical-syllabus";
...
function Section({
  ...
  return (
    <div className={accent ? "rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3" : undefined}>
      <div className="flex items-center gap-1.5 mb-2">
        {Icon && <Icon className={`h-4 w-4 ${accent ? "text-emerald-500" : "text-primary"}`} />}
        <h4 className={`text-xs font-semibold uppercase tracking-wider ${accent ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"}`}>
          {title}
        </h4>
      </div>
      {children}
...
function ExperimentCard({ experiment }: { experiment: PracticalExperiment }) {
  ...
  return (
    <div className="rounded-xl border border-border/70 bg-card overflow-hidden">
      ...
      >
        <span className="font-semibold text-foreground text-sm flex-1">{experiment.title}</span>
      ...
      {open && (
        <div className="px-5 pb-5 space-y-4 border-t border-border/50 pt-4">
          ...
          {experiment.requirement && experiment.requirement.length > 0 && (
            <Section icon={Beaker} title="Requirement / Apparatus & Chemicals">
              <div className="flex flex-wrap gap-2">
                {experiment.requirement.map((r) => (
                  <span key={r} className="inline-flex rounded-full bg-muted px-2.5 py-1 text-xs text-muted-foreground">
                    {r}
                  </span>
                ))}
              </div>
          ...
          {experiment.theory && experiment.theory.length > 0 && (
            <Section icon={BookOpen} title="Theory">
          ...
          {experiment.observation && experiment.observation.length > 0 && (
            <Section icon={Eye} title="Observation">
              <ul className="space-y-1.5">
            ...
            <Section icon={Calculator} title="Calculation">
              <div className="rounded-lg bg-muted p-3">
                {experiment.calculation.map((c, i) => (
                  <p key={i} className="text-sm text-foreground font-mono leading-relaxed mb-1 last:mb-0 whitespace-pre-wrap">{c}</p>
                ))}
              </div>
            </Section>
          )}
          ...
          {/* Result */}
          {experiment.result && (
            <Section icon={ScrollText} title="Result / Conclusion">
          ...
          {experiment.precautions && experiment.precautions.length > 0 && (
            <Section icon={ShieldAlert} title="Precautions" accent>
              <ul className="space-y-1.5">
...
export default function PracticalSubjectPage({
  ...
  const grouped = useMemo(() => {
    ...
    return subject.units.reduce(
      (acc, unit) => {
        (acc[unit.grade] = acc[unit.grade] ?? []).push(unit);
        return acc;
      },
  ...
  const totalExp = totalExperiments(subject);
    ...
    <div className="mx-auto max-w-4xl space-y-8 py-8 md:py-14 px-4">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-border bg-card p-6 sm:p-8">
        ...
        <div className="relative flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${subject.colorClass} text-2xl shadow-lg`}>
            {subject.emoji}
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground">{subject.name}</h1>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2.5 py-1">
                ...
                {subject.units.length} units · {totalExp} experiments
              </span>
            </div>
          </div>
          <Link href="/practical" className="shrink-0 text-sm text-muted-foreground hover:text-primary transition-colors">
            ← All practical subjects
          </Link>
        </div>
      </div>

      {/* Units grouped by grade */}
      {Object.entries(grouped).map(([grade, units]) => (
        <div key={grade} className="space-y-6">
          {/* Grade badge */}
          <div className="flex items-center gap-2">
            <span
              className={`inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r ${subject.colorClass} px-3 py-1.5 text-xs font-bold text-white shadow-sm`}
            >
              <Layers className="h-3.5 w-3.5" />
              {GRADE_LABELS[grade] ?? grade}
            </span>
            <span className="text-xs text-muted-foreground">
              {units.length} unit{units.length > 1 ? "s" : ""} · {units.reduce((s, u) => s + u.experiments.length, 0)} experiments
            </span>
          </div>
          ...
          {units.map((unit, ui) => (
            <div key={unit.id} className="space-y-4">
              <div className="flex items-center gap-2">
                <span className={`flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br ${subject.colorClass} text-xs font-bold text-white`}>
                  {ui + 1}
                </span>
