...
const SUBJECT_CONFIG: Record<LabCategory, { label: string; color: string; emoji: string }> = {
  ...
  class11: { label: "Class 11", color: "#f43f5e", emoji: "🎒" },
};
...
export default function Lab3DPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 py-8 md:py-14 px-4">
      <div>
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight">3D Simulation Lab</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Interactive 3D visualizations for physics, chemistry, math, and biology.
        </p>
      </div>
        ...
        {availableSubjects.map((subject) => {
          ...
          return (
            ...
            >
              <span>{cfg.emoji}</span>
              {cfg.label}
              <span className={`ml-2 text-xs px-1.5 py-0.5 rounded-md ${isActive ? "bg-white/20" : "bg-muted-foreground/20"}`}>
                {count}
              </span>
            </button>
          );
      ...
      {filteredLabs.length === 0 && (
        <div className="py-16 text-center text-muted-foreground">
          <p className="text-sm">No simulations found for {SUBJECT_CONFIG[activeSubject].label}.</p>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2">
        <span className="stat-pill">
          ...
          <span className="stat-pill-value">{LAB_REGISTRY.length}</span>
        </span>
