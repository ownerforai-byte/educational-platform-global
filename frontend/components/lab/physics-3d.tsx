          ...
          for (let i = 0; i < resolution; i++) {
            for (let j = 0; j < resolution; j++) {
              for (let k = 0; k < resolution; k++) {
                ...
                const arrow = new LiveLeaderLine(dir, origin, len, 0x22c55e, len * 0.3, len * 0.2);
          ...
          for (let i = 0; i < resolution; i++) {
            for (let j = 0; j < resolution; j++) {
              for (let k = 0; k < resolution; k++) {
                ...
                const arrow = new LiveLeaderLine(dir, origin, dir.length(), 0xfbbf24, dir.length() * 0.3, dir.length() * 0.2);
                const arrowAny = arrow as any;
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const rebuild = () => {
        ...
        // Vector A (red)
        if (A.length() > 0.05) {
          const arrowA = new LiveLeaderLine(A.clone().normalize(), new THREE.Vector3(0, 0, 0), A.length(), 0xef4444, 0.3, 0.2);
          vectorGroup.add(arrowA);
        }

        // Vector B (green)
        if (B.length() > 0.05) {
          const arrowB = new LiveLeaderLine(B.clone().normalize(), new THREE.Vector3(0, 0, 0), B.length(), 0x22c55e, 0.3, 0.2);
        ...
        if (showSum) {
          ...
          if (sum.length() > 0.05) {
            const arrowSum = new LiveLeaderLine(sum.clone().normalize(), new THREE.Vector3(0, 0, 0), sum.length(), 0xf59e0b, 0.35, 0.25);
        ...
        if (showCross) {
          ...
          if (cross.length() > 0.05) {
            const arrowCross = new LiveLeaderLine(cross.clone().normalize(), new THREE.Vector3(0, 0, 0), cross.length(), 0xa855f7, 0.35, 0.25);
