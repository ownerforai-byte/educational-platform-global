...
function MultivariableSurface() {
  ...
  useEffect(() => {
    ...
    const ts = createThreeScene(containerRef.current, { cameraPosition: new THREE.Vector3(9, 8, 10), autoRotate: true, autoRotateSpeed: 0.5 });
    tsRef.current = ts;
  ...
  return (
    <SimCard title="🧮 Multivariable Calculus — Surfaces & Contour Maps">
      ...
      <div ref={containerRef} className="w-full h-80 sm:h-96 md:h-[clamp(320px,60vh,640px)] lg:h-[clamp(320px,60vh,640px)] rounded-lg border border-border" aria-label="3D surface + contours" />
...
function VectorFieldDivCurl() {
  ...
  useEffect(() => {
    if (!containerRef.current || !isWebGLAvailable()) return;
    const ts = createThreeScene(containerRef.current, { cameraPosition: new THREE.Vector3(8, 8, 10), autoRotate: true, autoRotateSpeed: 0.5 });
    tsRef.current = ts;
    const unbind = bindResize(ts);
    function animate() {
    ...
    for (let i = -n; i <= n; i++) {
      for (let j = -n; j <= n; j++) {
        for (let k = -1; k <= 1; k += 2) {
          ...
          const arrow = new LiveLeaderLine(new THREE.Vector3(fx, fy, fz).normalize(), new THREE.Vector3(px, py, pz), len, color, len * 0.3, len * 0.2);
          ts!.group.add(arrow);
        }
      }
    }
  ...
  return (
    <SimCard title="🌀 Vector Calculus — Divergence & Curl">
      ...
      <div ref={containerRef} className="w-full h-80 sm:h-96 md:h-[clamp(320px,60vh,640px)] lg:h-[clamp(320px,60vh,640px)] rounded-lg border border-border" aria-label="3D vector field" />
      <p className="text-xs text-muted-foreground">
...
function MandelbulbFractal() {
  ...
  useEffect(() => {
    if (!containerRef.current || !isWebGLAvailable()) return;
    const ts = createThreeScene(containerRef.current, { cameraPosition: new THREE.Vector3(5, 4, 6), autoRotate: true, autoRotateSpeed: 0.8, grid: false });
    tsRef.current = ts;
    const unbind = bindResize(ts);
    function animate() {
      requestAnimationFrame(animate);
  ...
  return (
    <SimCard title="🧊 Fractals — 3D Mandelbulb">
      ...
      <div ref={containerRef} className="w-full h-80 sm:h-96 md:h-[clamp(320px,60vh,640px)] lg:h-[clamp(320px,60vh,640px)] rounded-lg border border-border" aria-label="3D Mandelbulb" />
      <p className="text-xs text-muted-foreground">
...
function ParametricSurface() {
  ...
  useEffect(() => {
    if (!containerRef.current || !isWebGLAvailable()) return;
    const ts = createThreeScene(containerRef.current, { cameraPosition: new THREE.Vector3(6, 4, 7), autoRotate: true, autoRotateSpeed: 0.6, grid: false });
    tsRef.current = ts;
    const unbind = bindResize(ts);
    function animate() {
      requestAnimationFrame(animate);
  ...
  return (
    <SimCard title="🪢 Differential Geometry — Parametric Surfaces">
      ...
      <div ref={containerRef} className="w-full h-80 sm:h-96 md:h-[clamp(320px,60vh,640px)] lg:h-[clamp(320px,60vh,640px)] rounded-lg border border-border" aria-label="3D parametric surface" />
      <p className="text-xs text-muted-foreground">
...
function MatrixTransforms() {
  ...
  useEffect(() => {
    if (!containerRef.current || !isWebGLAvailable()) return;
    const ts = createThreeScene(containerRef.current, { cameraPosition: new THREE.Vector3(7, 6, 9), autoRotate: true, autoRotateSpeed: 0.5, axes: true });
    tsRef.current = ts;
    const unbind = bindResize(ts);
    function animate() {
      requestAnimationFrame(animate);
  ...
  useEffect(() => {
    ...
    ts!.group.add(new LiveLeaderLine(new THREE.Vector3(1, 0, 0), origin, 2, 0xef4444, 0.4, 0.25));
    ts!.group.add(new LiveLeaderLine(new THREE.Vector3(0, 1, 0), origin, 2, 0x22c55e, 0.4, 0.25));
    ts!.group.add(new LiveLeaderLine(new THREE.Vector3(0, 0, 1), origin, 2, 0x3b82f6, 0.4, 0.25));
    // unit cube
    ...
    for (const [v, col] of basis) {
      ...
      const arrow = new LiveLeaderLine(new THREE.Vector3(tv[0], tv[1], tv[2]).normalize(), origin, Math.hypot(tv[0], tv[1], tv[2]), col, 0.4, 0.25);
      ts!.group.add(arrow);
    }
  ...
  return (
    <SimCard title="🧮 Linear Algebra — 3D Matrix Transformations">
      ...
      <div ref={containerRef} className="w-full h-80 sm:h-96 md:h-[clamp(320px,60vh,640px)] lg:h-[clamp(320px,60vh,640px)] rounded-lg border border-border" aria-label="3D matrix transform" />
      <p className="text-xs text-muted-foreground">
...
function RiemannSurface() {
  ...
  useEffect(() => {
    if (!containerRef.current || !isWebGLAvailable()) return;
    const ts = createThreeScene(containerRef.current, { cameraPosition: new THREE.Vector3(7, 5, 8), autoRotate: true, autoRotateSpeed: 0.6, grid: false });
    tsRef.current = ts;
    const unbind = bindResize(ts);
    function animate() {
      requestAnimationFrame(animate);
  ...
  return (
    <SimCard title="♾️ Complex Analysis — Riemann Surfaces">
      ...
      <div ref={containerRef} className="w-full h-80 sm:h-96 md:h-[clamp(320px,60vh,640px)] lg:h-[clamp(320px,60vh,640px)] rounded-lg border border-border" aria-label="3D Riemann surface" />
      <p className="text-xs text-muted-foreground">
...
function GameTheory3D() {
  ...
  return (
    <SimCard title="🎲 Game Theory — Payoff Matrix & Nash Equilibrium">
      <div ref={containerRef} className="w-full h-80 sm:h-96 md:h-[clamp(320px,60vh,640px)] lg:h-[clamp(320px,60vh,640px)] rounded-lg border border-border" aria-label="3D game theory" />
      <p className="text-xs text-muted-foreground">
...
function TopologyTwist() {
  ...
  useEffect(() => {
    if (!containerRef.current || !isWebGLAvailable()) return;
    const ts = createThreeScene(containerRef.current, { cameraPosition: new THREE.Vector3(6, 5, 7), autoRotate: true, autoRotateSpeed: 0.6, grid: false });
    tsRef.current = ts;
    const unbind = bindResize(ts);
    function animate() {
      requestAnimationFrame(animate);
  ...
  return (
    <SimCard title="🔄 Topology — Möbius Strip & Interactive Twist">
      ...
      <div ref={containerRef} className="w-full h-80 sm:h-96 md:h-[clamp(320px,60vh,640px)] lg:h-[clamp(320px,60vh,640px)] rounded-lg border border-border" aria-label="3D topology" />
      <p className="text-xs text-muted-foreground">
