...
function MagneticField3D() {
  ...
  useEffect(() => {
    ...
    const ts = createThreeScene(containerRef.current, { cameraPosition: new THREE.Vector3(1, 5, 11), autoRotate: true, autoRotateSpeed: 0.5 });
    tsRef.current = ts;
...
function WaveOptics3D() {
  ...
  useEffect(() => {
    if (!containerRef.current || !isWebGLAvailable()) return;
    const ts = createThreeScene(containerRef.current, { cameraPosition: new THREE.Vector3(10, 8, 14), autoRotate: true, autoRotateSpeed: 0.4, axes: true });
    tsRef.current = ts;
    const unbind = bindResize(ts);
    let rafId = 0;
    function animate() {
  ...
  useEffect(() => {
    ...
    for (let i = -3; i <= 3; i++) {
      const arrow = new LiveLeaderLine(new THREE.Vector3(1, 0, 0), new THREE.Vector3(-6.5, i * 0.9, 0), 1.4, 0x38bdf8, 0.4, 0.25);
      ts!.group.add(arrow);
    }
...
function SpacetimeCurvature() {
  ...
  useEffect(() => {
    if (!containerRef.current || !isWebGLAvailable()) return;
    const ts = createThreeScene(containerRef.current, { cameraPosition: new THREE.Vector3(9, 9, 13), autoRotate: true, autoRotateSpeed: 0.5 });
    tsRef.current = ts;
    const unbind = bindResize(ts);
    let rafId = 0;
    function animate() {
...
function QuantumOrbitals() {
  ...
  useEffect(() => {
    if (!containerRef.current || !isWebGLAvailable()) return;
    const ts = createThreeScene(containerRef.current, { cameraPosition: new THREE.Vector3(6, 5, 9), autoRotate: true, autoRotateSpeed: 0.7 });
    tsRef.current = ts;
    const unbind = bindResize(ts);
    let rafId = 0;
    function animate() {
...
function NuclearDecay() {
  ...
  useEffect(() => {
    if (!containerRef.current || !isWebGLAvailable()) return;
    const ts = createThreeScene(containerRef.current, { cameraPosition: new THREE.Vector3(6, 6, 9), autoRotate: true, autoRotateSpeed: 0.5 });
    tsRef.current = ts;
    const unbind = bindResize(ts);
    let rafId = 0;
    function animate() {
...
function FluidFlow() {
  ...
  useEffect(() => {
    if (!containerRef.current || !isWebGLAvailable()) return;
    const ts = createThreeScene(containerRef.current, { cameraPosition: new THREE.Vector3(12, 8, 12), autoRotate: true, autoRotateSpeed: 0.35 });
    tsRef.current = ts;
    const unbind = bindResize(ts);
    let rafId = 0;
    function animate() {
...
function NBodySystem() {
  ...
  useEffect(() => {
    if (!containerRef.current || !isWebGLAvailable()) return;
    const ts = createThreeScene(containerRef.current, { cameraPosition: new THREE.Vector3(12, 10, 12), autoRotate: true, autoRotateSpeed: 0.4 });
    tsRef.current = ts;
    const unbind = bindResize(ts);
    let rafId = 0;
    function animate() {
