...
function InclinedPlaneLab() {
  ...
  useEffect(() => {
    ...
    const weightLen = 3.2;
    ts!.group.add(new LiveLeaderLine(new THREE.Vector3(0, -1, 0), block.position.clone(), weightLen, 0xef4444, 0.45, 0.28));
    const normalDir = new THREE.Vector3(-Math.sin(theta), Math.cos(theta), 0);
    ts!.group.add(new LiveLeaderLine(normalDir, block.position.clone(), weightLen * Math.cos(theta), 0x22c55e, 0.45, 0.28));
    const frictionMag = muVal * weightLen * Math.cos(theta);
    if (frictionMag > 0.15) {
      ts!.group.add(new LiveLeaderLine(slideDir.clone().negate(), block.position.clone().add(new THREE.Vector3(0, 0.9, 0)), Math.min(3, frictionMag), 0x38bdf8, 0.4, 0.25));
    }
...
function MirrorOpticsLab() {
  ...
  useEffect(() => {
    ...
    clearGroup(ts!.group);
        ...
        const objX = -u * scale;
        const objH = 1.2;
        ts!.group.add(new LiveLeaderLine(new THREE.Vector3(0, 1, 0), new THREE.Vector3(objX, 0, 0), objH, 0x22c55e, 0.35, 0.22));
        ...
        if (Number.isFinite(imgX)) {
          ...
          ts!.group.add(new LiveLeaderLine(dir, new THREE.Vector3(imgX, 0, 0), imgH, 0xef4444, 0.35, 0.22));
        }
