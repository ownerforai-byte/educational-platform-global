...
function CoordinatePlane3D() {
  ...
  useEffect(() => {(async () => {
    ...
    const init = async () => {
      ...
      const vecArrow = new LiveLeaderLine(P.clone().normalize(), new THREE.Vector3(0, 0, 0), P.length(), 0x22d3ee, 0.25, 0.18);
      scene.add(vecArrow);
...
function CoordinateAxes3D() {
  ...
  useEffect(() => {(async () => {
    ...
    const init = async () => {
      ...
      const vecArrow = new LiveLeaderLine(
        P.clone().normalize(),
  ...
  useEffect(() => {(async () => {
    ...
    const init = async () => {
      ...
      const addVector = (v: { x: number; y: number; z: number }, color: number, origin: THREE.Vector3 = new THREE.Vector3(0, 0, 0)) => {
        ...
        const len = p.length();
        if (len < 0.05) return;
        const arrow = new LiveLeaderLine(p.clone().normalize(), origin, len, color, 0.3, 0.2);
        scene.add(arrow);
        meshes.push(arrow);
