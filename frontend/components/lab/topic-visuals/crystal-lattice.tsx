...
export function CrystalLatticeVisual() {
  ...
  const LATTICE_INFO: Record<LatticeType, { cell: string; atoms: string; packing: string; cn: string; example: string }> = {
    sc: { cell: "Simple Cubic (SC)", atoms: "1 per cell", packing: "52%", cn: "6", example: "Polonium (Po)" },
    bcc: { cell: "Body-Centered Cubic (BCC)", atoms: "2 per cell", packing: "68%", cn: "8", example: "Fe, Na, K" },
    fcc: { cell: "Face-Centered Cubic (FCC)", atoms: "4 per cell", packing: "74% (closest packing)", cn: "12", example: "Cu, Ag, Au, Al" },
  };
  const info = LATTICE_INFO[lattice];

  const DEFAULTS = { lattice: "fcc" as LatticeType };
  const presets: ScenePreset[] = [
    {
      name: "Simple cubic",
      hint: "Atoms only at the 8 cube corners — loosest packing at 52%.",
      apply: () => { setLattice("sc"); setRunId((r) => r + 1); },
    },
    {
      name: "Body-centered (BCC)",
      hint: "Corners + one atom at the body center, 68% packed.",
      apply: () => { setLattice("bcc"); setRunId((r) => r + 1); },
    },
    {
      name: "Face-centered (FCC)",
      hint: "Corners + 6 face centers — the densest common packing, 74%.",
      apply: () => { setLattice("fcc"); setRunId((r) => r + 1); },
    },
  ];

  const resetAll = () => {
    setLattice(DEFAULTS.lattice);
    setShowLabels(true);
    setRunId((r) => r + 1);
  };
    ...
    const init = async () => {
      ...
      const updateScene = () => {
        ...
        if (lattice === "fcc") {
          ...
          addLabel(mkSprite("Face-centered atom (green)", "#22c55e", labelPos.clone().sub(dir.multiplyScalar(0.5)), 0.7));
          ...
          addLabel(mkSprite("Corner atom (blue)", "#3b82f6", cLabelPos.clone().sub(cDir.multiplyScalar(0.5)), 0.7));
        }

        // Info label
        addLabel(mkSprite(`${atomType}  |  Packing: ${packingEff}  |  CN: ${cn}`, "#fbbf24", new THREE.Vector3(0, -2.0, 0), 0.7));
        ...
        if (lattice === "fcc") {
          ...
          addLabel(mkSprite("Unit cell edge = a (lattice parameter)", "#f97316", labelPos2.clone().sub(dir2.multiplyScalar(0.5)), 0.65));
        }
      };
  ...
  }, [lattice, isWebGL, runId, showLabels]);
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <ReadoutGrid
          items={[
            { label: "Unit cell", value: info.cell },
            { label: "Atoms per cell", value: info.atoms },
            { label: "Packing efficiency", value: info.packing, highlight: true },
