...
export function NuclearFissionVisual() {
  ...
  const relaunch = (nextSpeed: number) => {
    setSpeed(nextSpeed);
    setAnimating(true);
    setFissioned(false);
    setRunId((r) => r + 1);
  ...
  const presets: ScenePreset[] = [
    {
      name: "Thermal neutron (0.25×)",
      hint: "Slow-motion capture — trace the neutron's approach and the fragments' separation.",
      apply: () => relaunch(0.25),
    },
    {
      name: "Standard fission (1×)",
      hint: "Typical event: the nucleus absorbs a neutron, splits and releases ≈200 MeV.",
      apply: () => relaunch(1),
    },
    {
      name: "Fast neutron (2×)",
      hint: "Double speed — fragments and prompt neutrons fly apart quickly.",
      apply: () => relaunch(2),
    },
  ];
  ...
  useEffect(() => {
    ...
    // The scene always relaunches the neutron; keep the readout state in sync.
    setFissioned(false);
    ...
    const init = async () => {
      ...
      addLabel(mkSprite("n (neutron)", "#94a3b8", nLabelPos.clone().sub(nDir.multiplyScalar(0.5)), 0.75));
      ...
      addLabel(mkSprite("²³⁸U (uranium nucleus)", "#f97316", uLabelPos.clone().sub(uDir.multiplyScalar(0.5)), 0.8));
      ...
      addLabel(mkSprite("Energy released ≈ 200 MeV", "#ef4444", energyLabelPos.clone().sub(energyDir.multiplyScalar(0.5)), 0.75));
      ...
      const animate = () => {
        ...
        if (animating) {
          ...
          if (!hasFissioned) {
            ...
            fragments.forEach((f) => {
              f.mesh.position.addScaledVector(f.vel, step);
            });
            neutrinos.forEach((nv) => {
              nv.position.addScaledVector(nv.userData.vel, step);
            });
          }
        }
  ...
  }, [isWebGL, runId, animating, showLabels]);
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <ReadoutGrid
          items={[
            { label: "Energy released", value: fissioned ? "≈200" : "0", unit: "MeV", highlight: fissioned },
            { label: "Prompt neutrons", value: fissioned ? 3 : 0, highlight: fissioned },
            { label: "Reaction status", value: fissioned ? "Fissioned" : "Awaiting neutron" },
            { label: "Critical mass (²³⁵U)", value: "52", unit: "kg" },
          ]}
        />
