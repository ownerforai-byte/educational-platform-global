...
/* ============================================================
   Vector Addition 3D — NEB Analytic Geometry & Vectors (Maths 11)
...
const MODE_INFO: Record<VectorMode, { concept: string; formula: string; interpretation: string; fact: string; tip: string }> = {
  addition: {
    ...
    interpretation: "Place the tail of B at the head of A (triangle law); the resultant runs from the free tail to the free head — it is the diagonal of the parallelogram built on A and B.",
    fact: "Vector addition is commutative and associative: A + B = B + A and (A + B) + C = A + (B + C), so the order never changes the resultant.",
  ...
  scalar: {
    ...
    fact: "k = 0 collapses any vector to the zero vector, which has no fixed direction.",
  ...
  "linear-combo": {
    ...
    fact: "Two non-collinear vectors span a plane; three non-coplanar vectors span all of space and form a basis (like î, ĵ, k̂).",
    tip: "If c₁A + c₂B + c₃C = 0 forces c₁ = c₂ = c₃ = 0, the vectors are linearly independent; any other solution means dependence.",
  },
};

export function VectorAddition3DVisual() {
  ...
  const presets: ScenePreset[] = [
    { name: "Parallelogram law", hint: "A + B is the diagonal of the parallelogram built on A and B", apply: () => { setMode("addition"); setA({ x: 3, y: 1, z: 0 }); setB({ x: 1, y: 3, z: 0 }); setRunId((r) => r + 1); } },
    { name: "Triangle in 3D", hint: "Head-to-tail addition works out of the plane too", apply: () => { setMode("addition"); setA({ x: 2, y: 0, z: 1 }); setB({ x: 0, y: 3, z: 1 }); setRunId((r) => r + 1); } },
    { name: "Negative vector (k = −1)", hint: "−A has the same length but the opposite direction", apply: () => { setMode("scalar"); setK(-1); setRunId((r) => r + 1); } },
    { name: "Linear combo 1.5A + 0.8B", hint: "Any combination of A and B stays in their plane", apply: () => { setMode("linear-combo"); setRunId((r) => r + 1); } },
  ];
  ...
  if (!isWebGL) {
    return <WebGLFallback title="Vector Addition 3D" description="Interactive 3D vector visualization — requires WebGL." />;
  }
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex flex-wrap items-center gap-2">
          <span>Vector Addition 3D — Addition, Collinearity & Coplanarity</span>
