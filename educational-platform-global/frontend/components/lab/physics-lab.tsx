        ...
        <CollapsibleControls label="Motion Parameters">
          <div className="flex flex-wrap items-center gap-3">
            <div className="w-44">
              ...
              <Input type="number" value={radius} onChange={(e) => setRadius(Number(e.target.value))} className="mt-1" />
            </div>
            <div className="w-44">
              <Label className="text-xs text-muted-foreground">Angular Speed (ω)</Label>
              <Input type="number" step="0.1" value={speed} onChange={(e) => setSpeed(Number(e.target.value))} className="mt-1" />
            </div>
            <Button variant={showVelocity ? "default" : "outline"} size="sm" onClick={() => setShowVelocity(!showVelocity)}>
              Velocity
            </Button>
            <Button variant={showCentripetal ? "default" : "outline"} size="sm" onClick={() => setShowCentripetal(!showCentripetal)}>
              Centripetal Accel
            </Button>
          </div>
        </CollapsibleControls>
        ...
        <CollapsibleControls label="SHM Parameters">
          <div className="flex flex-wrap items-center gap-3">
            <div className="w-44">
              ...
              <Input type="number" value={amplitude} onChange={(e) => setAmplitude(Number(e.target.value))} className="mt-1" />
            </div>
            <div className="w-44">
              <Label className="text-xs text-muted-foreground">Frequency (Hz)</Label>
              <Input type="number" step="0.1" value={frequency} onChange={(e) => setFrequency(Number(e.target.value))} className="mt-1" />
            </div>
            <Button variant={showTimeGraph ? "default" : "outline"} size="sm" onClick={() => setShowTimeGraph(!showTimeGraph)}>
              Time graph
            </Button>
          </div>
        </CollapsibleControls>
  ...
  return (
    <Tabs defaultValue="projectile" className="w-full">
      <TabsList className="flex-wrap">
        ...
        <TabsTrigger value="shm">SHM</TabsTrigger>
      </TabsList>
