      ...
      <CardContent className="space-y-4">
        <CollapsibleControls label="Function Options">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
            ...
            <Button onClick={() => updateFunction(fnExpr)}>Plot</Button>
          </div>
          <div className="flex flex-wrap gap-2">
            {["x^2", "x^3", "sin(x)", "cos(x)", "2*x+3", "x^2-4", "abs(x)", "1/x"].map((preset) => (
              <Button key={preset} variant="outline" size="sm" onClick={() => updateFunction(preset)}>
                {preset}
              </Button>
            ))}
          </div>
        ...
        <MeaningPanel
          ...
          points={[
            "f(x) + k â†’ shift up k â€¢ f(x) âˆ’ k â†’ shift down k",
            "f(x âˆ’ h) â†’ shift right h â€¢ f(x + h) â†’ shift left h",
            "âˆ’f(x) â†’ reflection in x-axis â€¢ f(âˆ’x) â†’ reflection in y-axis",
            "y = axÂ² + bx + c: |a| > 1 stretches, |a| < 1 compresses; a < 0 reflects",
...
if (!isWebGLAvailable()) return; const renderer = new THREE.WebGLRenderer({ antialias: true });
  ...
  return (
    <div className="space-y-3">
      <div ref={containerRef} className="lab-3d-container rounded-md border border-border" aria-label="Interactive 3D coordinate geometry" />
...
function MathSurfaces3D() {
  ...
  const surfaceLabels: Record<string, string> = {
    saddle: "Saddle (z = xÂ² âˆ’ yÂ²)",
    wave: "Wave (z = sin(x)Â·cos(y))",
    ripple: "Ripple (z = sin(âˆš(xÂ²+yÂ²)))",
    peak: "Peak (z = e^âˆ’(xÂ²+yÂ²)/8)",
  ...
  return (
    <div className="space-y-3">
      <div ref={containerRef} className="lab-3d-container rounded-md border border-border" aria-label="3D mathematical surfaces" />
      ...
      <MeaningPanel
        ...
        points={[
          "Plane: z = ax + by + c â†’ flat surface, slope determined by a and b",
          "Paraboloid: z = xÂ² + yÂ² â†’ like a bowl opening upward",
          "Saddle: z = xÂ² âˆ’ yÂ² â†’ curves up one way, down the other",
          "Wave: z = sin(x)Â·cos(y) â†’ periodic in both directions",
...
if (!isWebGLAvailable()) return; const renderer = new THREE.WebGLRenderer({ antialias: true });
  ...
  return (
    <div ref={containerRef} className="lab-3d-container rounded-md border border-border" aria-label="Interactive 3D parabola visualization" />
  );
}
      ...
      <CardContent className="space-y-4">
        <Tabs value={tab} onValueChange={setTab} className="w-full">
          <TabsList className="flex-wrap">
            <TabsTrigger value="geometry">Coordinate Geometry</TabsTrigger>
            <TabsTrigger value="graph">Function Graphs</TabsTrigger>
            <TabsTrigger value="surface">3D Surfaces</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="graph" className="mt-4">
            <div className="space-y-6">
              ...
              <Card>
                <CardHeader>
                  <CardTitle>3D Parabola (y = xÂ²)</CardTitle>
                </CardHeader>
                <CardContent>
                  <Parabola3D />
                </CardContent>
