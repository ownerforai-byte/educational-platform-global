/**
 * Script to add tabbed structure to all physics concept files
...
const baseDir = path.join(__dirname, '../content/ravikishan/class-11-notes/physics');
...
// Process all physics directories
const units = ['dynamics', 'elasticity', 'electric-charges', 'electric-field', 'gravitation', 'work-energy-power'];
let totalFiles = 0;
...
for (const unit of units) {
  ...
  if (fs.existsSync(unitDir)) {
    ...
    processDirectory(unitDir);
  }
}

console.log(`\n✓ Processed ${totalFiles} physics files!`);
