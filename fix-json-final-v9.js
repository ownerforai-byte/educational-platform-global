  ...
  while (i < content.length) {
    ...
    if (!inString) {
      ...
      if (prevInputWasBackslash) {
        ...
        // Check if this is a string TERMINATOR first
        const bsCount = countLeadingBackslashes(content, i);
        if (bsCount % 2 === 0 && isStringTerminator(content, i)) {
          // Terminator: end the string
          result += ch;
          inString = false;
          prevInputWasBackslash = false;
          i++;
        } else {
          // Not a terminator - unescaped quote inside string, ESCAPE IT
          result += '\\"';
          prevInputWasBackslash = false;
          i++;
        }
