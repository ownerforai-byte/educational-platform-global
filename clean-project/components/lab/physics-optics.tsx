...
export const OPTICAL_MEDIUMS = [
  { name: "Vacuum", n: 1.0, color: "#0f172a", description: "Perfect vacuum — speed of light is absolute maximum (3.00 × 10⁸ m/s). Reference medium where n = 1 exactly." },
  { name: "Air", n: 1.0003, color: "#1e293b", description: "Standard atmosphere (1 atm, 20°C). Light slows by only 0.03% — for most school problems air is treated as n ≈ 1." },
  { name: "Water", n: 1.333, color: "#0ea5e9", description: "Pure water at 20°C. Light slows to ~75% of its vacuum speed. Causes noticeable bending when light enters from air." },
  { name: "Ice", n: 1.309, color: "#a5f3fc", description: "Crystalline ice (H₂O solid). Slightly less dense optically than liquid water; vitreous ice ~1.31–1.31." },
  { name: "Glass slab (crown)", n: 1.52, color: "#38bdf8", description: "Crown glass (SiO₂ + Na₂O + CaO, ~72% SiO₂). The classic 'glass slab' in Lab experiments — lateral shift is easy to observe." },
  { name: "Glass slab (flint)", n: 1.62, color: "#818cf8", description: "Flint glass (adds PbO) — higher dispersion, used in prisms and achromatic lenses." },
  { name: "Plexiglass (acrylic)", n: 1.49, color: "#67e8f9", description: "PMMA acrylic. Common in optics kits and lens experiments; n ≈ 1.48–1.50." },
  { name: "Glycerin", n: 1.473, color: "#fbbf24", description: "Viscous organic liquid; used to demonstrate refraction and density layers." },
  { name: "Quartz (fused silica)", n: 1.458, color: "#e2e8f0", description: "Pure SiO₂ glass. Low thermal expansion, used in UV optics and optical fibers." },
  { name: "Diamond", n: 2.417, color: "#34d399", description: "Highest natural refractive index. Critical angle only ~24.4° → brilliant total internal reflection, the reason diamonds sparkle." },
  { name: "Sapphire", n: 1.768, color: "#3b82f6", description: "Al₂O₃ crystal; hard, durable, used in watch crystals and high-end optics." },
  { name: "Ruby", n: 1.770, color: "#f43f5e", description: "Al₂O₃ doped with Cr³⁺; same index as sapphire (~1.77), reddish colour." },
  { name: "Carbon disulfide", n: 1.628, color: "#fde047", description: "CS₂ liquid — high index, highly dispersive; classic prism demo liquid." },
  { name: "Benzene", n: 1.501, color: "#94a3b8", description: "Aromatic liquid; index ≈ 1.50, close to crown glass." },
  ...
  { name: "Sea water (3.5% salinity)", n: 1.343, color: "#0ea5e9", description: "Salt water at 20°C, 3.5% salinity. Slightly denser than pure water — refraction increases with salinity, used by sailors for crude refraction-based depth readings." },
  { name: "Sugar solution (30%)", n: 1.38, color: "#fcd34d", description: "Sucrose syrup at 30% concentration, n ≈ 1.38. Higher sugar = higher index — the principle behind the sugar refractometer." },
  { name: "Glass BK7 (borosilicate)", n: 1.517, color: "#7dd3fc", description: "BK7 — premium optical glass (Schott). Standard for lab lenses, n = 1.517 at 587.6 nm." },
  { name: "Soda-lime glass", n: 1.52, color: "#38bdf8", description: "Everyday window glass ≈ 1.52. Same as crown glass — used in most school 'glass slab' experiments." },
  { name: "Dense flint (SF10)", n: 1.728, color: "#818cf8", description: "SF10 glass — very high dispersion and index 1.728. Used in high-power achromatic lenses." },
  { name: "Polycarbonate", n: 1.585, color: "#c4b5fd", description: "PC plastic ≈ 1.585. Strong, impact-resistant; used in lenses, safety glasses, and CD/DVDs." },
  { name: "Salt (halite, NaCl)", n: 1.544, color: "#f0abfc", description: "Rock-salt crystal n = 1.544. Transparent in the infrared — used in IR spectroscopy windows." },
  { name: "Zircon", n: 1.92, color: "#fda4af", description: "ZrSiO₄ mineral, n ≈ 1.92. A cheap diamond simulant with noticeable dispersion." },
  { name: "Naphthalene", n: 1.589, color: "#fbbf24", description: "C₁₀H₈ crystal, n = 1.589. Highly dispersive aromatic solid." },
  { name: "Methanol", n: 1.329, color: "#67e8f9", description: "Methyl alcohol — index 1.329, very close to water." },
  { name: "Acetone", n: 1.359, color: "#94a3b8", description: "C₃H₆O solvent, n = 1.359. Similar to ethanol." },
] as const;
...
function ReflectionLab() {
  ...
  useEffect(() => {
    ...
    ctx.fillText("Incident angle " + angle + "°", cx - 120, cy + 78);
    ctx.fillStyle = "#22d3ee";
    ctx.fillText("Reflected angle " + angle + "°", cx + 10, cy + 78);
...
function RefractionLab() {
  ...
  useEffect(() => {
    ...
    ctx.fillText(`${medium1.name}  (n₁ = ${n1})`, cx - 180, 24);
    ctx.fillText(`${medium2.name}  (n₂ = ${n2})`, cx - 180, H - 16);
    ctx.fillStyle = "#fbbf24";
    ctx.font = "600 14px Inter, sans-serif";
    ctx.fillText("θ₁ = " + angle + "°", cx - 190, cy - 78);
    if (cAngle !== null) {
      ctx.fillStyle = "#f8fafc";
      ctx.font = "400 13px Inter, sans-serif";
      ctx.fillText("Critical angle = " + cAngle.toFixed(1) + "°", cx - 180, cy + 88);
    }
    if (theta2 > 0 && !isTIR) {
      ctx.fillStyle = "#22d3ee";
      ctx.font = "500 13px Inter, sans-serif";
      ctx.fillText("θ₂ = " + ((theta2 * 180) / Math.PI).toFixed(1) + "° (Snell's law)", cx + 40, cy + 44);
    }

    // Info box: speed ratio
    ctx.fillStyle = "rgba(255,255,255,0.75)";
    ctx.font = "400 12px Inter, sans-serif";
    ctx.fillText("Speed ratio v₁/v₂ = n₂/n₁ ≈ " + (n2 / n1).toFixed(3), 16, H - 36);
  }, [n1, n2, angle, medium1, medium2]);

  const critical = n1 > n2 ? ((Math.asin(n2 / n1) * 180) / Math.PI).toFixed(1) : "—";

  return (
    <Card>
      <CardHeader>
        <CardTitle>Refraction — Snell&rsquo;s Law & TIR</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <div className="space-y-1">
            <Label htmlFor="refraction-angle" className="text-xs font-semibold text-foreground">θ₁ (°)</Label>
        ...
        <p className="text-xs text-muted-foreground">
          Pick a medium preset above — each has a real refractive index. Try Glass slab (n = 1.52) → Air (n ≈ 1): as θ₁ passes the critical angle ({critical}°), the ray reflects internally instead of refracting.
        </p>
      </CardContent>
    </Card>
  );
...
function MediumExplorer() {
  ...
  useEffect(() => {
    ...
    // The "light beam" is drawn as a straight line — in this explorer the
    ...
    // Speed of light bar (relative speed v = c₀/n)
    const v = 1 / medium.n;
    const barMax = W - 240;
    ...
    ctx.fillStyle = "#fbbf24";
    ctx.font = "500 14px Inter, sans-serif";
    ctx.fillText("n = " + medium.n + "   v = " + (v * 3.0e8).toExponential(1).replace("e+", "×10^") + " m/s", 40, 84);
    ...
    ctx.fillText("c₀ (300,000 km/s)", 40 + barMax - 110, H / 2 + 62);
...
function LateralShiftLab() {
  ...
  useEffect(() => {
    ...
    const rad = (angle * Math.PI) / 180;
    // Snell for entry into slab (air → glass)
    ...
    ctx.fillStyle = "#fbbf24";
    ctx.font = "600 14px Inter, sans-serif";
    ctx.fillText("θ₁ = " + angle + "°", entryX - 140, entryY - 40);
    ctx.fillStyle = "#22d3ee";
    ctx.fillText("θ₂ = " + ((r * 180) / Math.PI).toFixed(1) + "°", exitX + 10, entryY + 20);
    ctx.fillStyle = "#f87171";
    ctx.font = "500 13px Inter, sans-serif";
    ctx.fillText("Lateral shift d ≈ " + shift.toFixed(1) + " px", exitX - 40, entryY - 6);

    // Formula bar
    ctx.fillStyle = "#e2e8f0";
    ctx.font = "400 12px Inter, sans-serif";
    ctx.fillText("d = t·sin(θ₁ − θ₂) / cos(θ₂)", 40, H - 24);
  }, [n, thickness, angle]);

  const critical = n > 1 ? ((Math.asin(1 / n) * 180) / Math.PI).toFixed(1) : "—";

  return (
    <Card>
      <CardHeader>
        <CardTitle>Lateral Shift — Glass Slab</CardTitle>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <div className="space-y-1">
            <Label htmlFor="slab-angle" className="text-xs font-semibold text-foreground">Angle of incidence (°)</Label>
        ...
        <p className="text-xs text-muted-foreground">
          The emergent ray is parallel to the incident ray. Lateral shift increases with thickness, and with refractive index (compare n = 1.33 water vs n = 2.42 diamond). Critical angle in this medium ≈ {critical}° — beyond that, light reflects instead of escaping.
        </p>
      </CardContent>
    </Card>
  );
...
function PrismDispersion() {
  ...
  useEffect(() => {
    ...
    ctx.fillStyle = "#94a3b8";
    ctx.fillText("Refracting angle: " + angle + "°", W - 210, 28);
  }, [angle]);
...
function DiffractionLab() {
  ...
  useEffect(() => {
    ...
    ctx.fillText("λ = " + wavelength + " nm", screenX - 24, 20);
  }, [wavelength, slits]);
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-xs text-muted-foreground">Single slit → central bright + fading fringes; double slit → fine interference fringes inside a sinc envelope.</span>
        </div>
      </CardContent>
    </Card>
  );
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-xs text-muted-foreground">Lower frequency = longer wavelength → wider fringes.</span>
        </div>
      </CardContent>
    </Card>
  );
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-md border border-primary/20 bg-primary/5 p-3">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            The lens formula 1/f = 1/v − 1/u governs image formation. A convex lens (f {'>'} 0) converges light; a concave lens (f {'<'} 0) diverges it. The power P = 1/f in dioptres.
          </p>
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Refracting Angle (°)</Label>
        ...
        <p className="text-xs text-muted-foreground">
          White light entering a prism is dispersed into its constituent colours. Violet (short wavelength) bends most; red (long wavelength) bends least. The deviation δ ≈ (n − 1)·A for a thin prism.
        </p>
        <div className="rounded-md border border-primary/20 bg-primary/5 p-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-primary">📘 Observation & Conclusion</p>
          <h4 className="mt-1 text-sm font-semibold">What you see</h4>
          ...
          <h4 className="mt-2 text-sm font-semibold">Conclusion</h4>
          <p className="mt-1 text-xs text-muted-foreground">
            Dispersion occurs because the refractive index depends on wavelength: n_violet {'>'} n_red. The dispersive power ω = (n_violet − n_red)/(n_yellow − 1) measures how strongly a material spreads colours.
          </p>
          <h4 className="mt-2 text-sm font-semibold">Why it matters</h4>
          <p className="mt-1 text-xs text-muted-foreground">
