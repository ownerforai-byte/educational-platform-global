...
export default function LabPage() {
  ...
  return (
    <div className="container mx-auto max-w-7xl space-y-6 py-6 sm:py-10 px-4 sm:px-6">
      {/* Header */}
      <div className="space-y-2">
        ...
        <div className="flex gap-3 mt-4 flex-wrap">
          <Link href="/lab/3d" className="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-500/10 text-blue-700 border border-blue-500/20 hover:bg-blue-500/20 transition-colors text-sm font-medium">
            <Cuboid className="h-4 w-4" />
            <span>Open 3D Lab (Fullscreen)</span>
          </Link>
          <Link href="/lab/theory" className="flex items-center gap-2 px-4 py-2 rounded-lg bg-green-500/10 text-green-700 border border-green-500/20 hover:bg-green-500/20 transition-colors text-sm font-medium">
            <BookOpen className="h-4 w-4" />
            <span>Open Theory Lab (Fullscreen)</span>
          </Link>
          <Link href="/lab/class11" className="flex items-center gap-2 px-4 py-2 rounded-lg bg-purple-500/10 text-purple-700 border border-purple-500/20 hover:bg-purple-500/20 transition-colors text-sm font-medium">
            <GraduationCap className="h-4 w-4" />
            <span>Open Class 11 Lab (Fullscreen)</span>
          </Link>
        </div>
      </div>
      ...
      <Tabs value={activeSubject} onValueChange={(v) => setActiveSubject(v as LabCategory)} className="w-full">
        <TabsList className="flex-wrap h-auto bg-transparent border-b rounded-none px-0 w-full justify-start gap-1">
          <TabsTrigger value="physics" className="gap-2 py-2 px-4 data-[state=active]:bg-primary/10 data-[state=active]:text-primary">
            <Atom className="h-4 w-4" />
            <span>Physics</span>
            <span className="text-xs opacity-60">({labs.filter(l => l.category === "physics").length})</span>
          </TabsTrigger>
          <TabsTrigger value="chemistry" className="gap-2 py-2 px-4 data-[state=active]:bg-primary/10 data-[state=active]:text-primary">
            <FlaskConical className="h-4 w-4" />
            <span>Chemistry</span>
            <span className="text-xs opacity-60">({labs.filter(l => l.category === "chemistry").length})</span>
          </TabsTrigger>
          <TabsTrigger value="mathematics" className="gap-2 py-2 px-4 data-[state=active]:bg-primary/10 data-[state=active]:text-primary">
            <Calculator className="h-4 w-4" />
            <span>Mathematics</span>
            <span className="text-xs opacity-60">({labs.filter(l => l.category === "mathematics").length})</span>
          </TabsTrigger>
          <TabsTrigger value="biology" className="gap-2 py-2 px-4 data-[state=active]:bg-primary/10 data-[state=active]:text-primary">
            <Dna className="h-4 w-4" />
            <span>Biology</span>
            <span className="text-xs opacity-60">({labs.filter(l => l.category === "biology").length})</span>
          </TabsTrigger>
          <TabsTrigger value="class11" className="gap-2 py-2 px-4 data-[state=active]:bg-primary/10 data-[state=active]:text-primary">
            <GraduationCap className="h-4 w-4" />
            <span>Class 11</span>
          </TabsTrigger>
        </TabsList>
            ...
            {SelectedLabComponent ? (
              <div className="bg-card border border-border rounded-xl overflow-hidden">
                ...
                <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-muted/30">
                  {currentLab?.icon && (
                    <span className="text-primary">{currentLab.icon}</span>
                  )}
                  <div className="flex-1 min-w-0">
                    <h2 className="font-semibold text-lg truncate">{currentLab?.title}</h2>
                    <p className="text-xs text-muted-foreground truncate">{currentLab?.description}</p>
                  </div>
                  {currentLab && (
                    <span className={`shrink-0 text-xs px-2 py-0.5 rounded-full border ${
                      ...
                      {currentLab.status === "new" ? "New" :
                       currentLab.status === "premium" ? "Premium" :
                       currentLab.status === "development" ? "Dev" : "Active"}
                    </span>
                  )}
                </div>
              ...
              <div className="bg-card border border-border rounded-xl p-12 text-center">
                ...
                <h2 className="text-lg font-semibold mb-2">Select a Lab</h2>
