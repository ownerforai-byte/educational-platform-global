...
async function loadNoteGroups(): Promise<NoteGroup[]> {
  ...
  for (const subject of SUBJECTS) {
    const notes = await getImportedNotesForSubject(subject.slug, "class-12-notes");
    if (notes.length > 0) {
...
export default async function Class12NotesPage() {
    ...
    <div className="mx-auto max-w-5xl space-y-8 py-10">
      <div className="rounded-2xl border border-border bg-gradient-to-br from-primary/10 via-background to-background p-8 shadow-lg">
        ...
        <p className="mt-2 text-muted-foreground">
          NEB Class 12 subjects — imported notes organized by syllabus unit.
        </p>
      </div>
        ...
        {groups.map((group) => (
          <section key={group.subject} id={`subject-${group.subject}`}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <Link
                href={`/class-12-notes/${group.subject}`}
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                No imported notes are available for Class 12 yet. Check back later.
              </p>
            </CardContent>
          </Card>
        )}
