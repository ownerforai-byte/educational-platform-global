...
function LiquidHeatExperiment() {
  ...
  // Energy balance: heat lost by hot solid = heat gained by liquid + calorimeter water equivalent.
  // θ = (c_s·m_s·T_s + (c_L·m_L + c_w·m_w)·T_L) / (c_s·m_s + c_L·m_L + c_w·m_w)
  const numerator = solidC * solidMass * solidTemp + (liquid.c * liquidMass + cw * waterEq) * liquidTemp;
...
function PhaseChange3D() {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {error ? <WebGLFallback /> : <div ref={containerRef} className="h-[420px] w-full rounded-md border border-border" />}
        <div className="flex flex-wrap items-center gap-4">
          <div className="space-y-1">
