      ...
      <CardContent className="space-y-4">
        ...
        <MeaningPanel
          ...
          points={[
            ...
            "Some deflected at large angles → a concentrated positive charge (nucleus) exists",
            "Very few bounced back → nucleus is extremely dense and tiny",
      ...
      <CardContent className="space-y-4">
        ...
        <CollapsibleControls label="Uncertainty Options">
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              <Label htmlFor="posUnc">Position Uncertainty (Δx)</Label>
        ...
        <p className="text-xs text-muted-foreground">Cyan cloud = position uncertainty (Δx). Red wave = momentum uncertainty (Δp). Larger cloud → tighter wave and vice versa.</p>
        <MeaningPanel
          title="Heisenberg Uncertainty Principle (1927)"
          meaning="You cannot simultaneously know both the exact position and exact momentum of a particle. The product of their uncertainties is always ≥ h/4π (Planck's constant)."
          points={[
            "Δx · Δp ≥ h/4π — the fundamental limit of measurement",
            "Narrow position (small Δx) → broad momentum spread (large Δp)",
            "Wide position (large Δx) → narrow momentum spread (small Δp)",
...
const SPECTRAL_SERIES: SeriesDef[] = [
  ...
  { id: "balmer", name: "Balmer Series", nFinal: 2, region: "Visible Light", color: 0x22d3ee, description: "Transitions to n=2. Visible spectrum — the famous hydrogen lines (Hα, Hβ, Hγ, Hδ)." },
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <MeaningPanel
          ...
          meaning="When an electron drops from a higher energy level (n₂) to a lower one (n₁), it emits a photon of specific wavelength. Each series corresponds to a fixed final level n₁."
          points={[
            "1/λ = R(1/n₁² − 1/n₂²) — Rydberg formula (R = 1.097 × 10⁷ m⁻¹)",
            "Lyman (n₁=1): UV • Balmer (n₁=2): visible • Paschen (n₁=3): IR",
            "Brackett (n₁=4), Pfund (n₁=5), Humphreys (n₁=6): far infrared",
            "Balmer series produces the 4 visible lines: Hα (red), Hβ (cyan), Hγ (blue), Hδ (violet)",
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground font-normal">Drag to rotate • Scroll to zoom • Hover/click elements</span>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <MeaningPanel
          ...
          points={[
            ...
            "Metals (left) → metalloids (staircase) → nonmetals (right)",
