...
function PendulumSimulator() {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <canvas ref={canvasRef} className="h-[300px] w-full rounded-md border border-border bg-background" aria-label="Pendulum simulator" />
        <ControlPanel
          groups={[
            {
              id: "pend-length",
...
function LensCalculator() {
  ...
  const calculate = () => {
    ...
    // Cartesian sign convention (u < 0):
    //   Lens:   1/v − 1/u = 1/f  →  v = u·f / (u + f),  magnification m = v/u
    //   Mirror: 1/v + 1/u = 1/f  →  v = u·f / (u − f),  magnification m = −v/u
    const v = mode === "lens" ? (u * f) / (u + f) : (u * f) / (u - f);
    if (!Number.isFinite(v)) { setResult("Image is formed at infinity (object placed at the focal point)"); return; }
    const m = mode === "lens" ? v / u : -v / u;
    const type =
      mode === "lens"
        ? f > 0 ? "Convex (Converging)" : "Concave (Diverging)"
        : f > 0 ? "Concave (Converging)" : "Convex (Diverging)";
