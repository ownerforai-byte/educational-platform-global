...
router.get("/", requireAuth, async (req: Request, res: Response) => {
  ...
  const session =
    typeof req.query.session === "string" ? req.query.session.slice(0, 64) : "default";
  const limitRaw = Number(req.query.limit);
  const limit = Number.isFinite(limitRaw)
    ? Math.min(Math.max(Math.trunc(limitRaw), 1), 500)
    : 200;

  // Fetch NEWEST-first then re-reverse: sorting ascending before the limit
  // returned the OLDEST 200 once a session grew past the restore cap,
...
router.delete("/", requireAuth, async (req: Request, res: Response) => {
  const user = (req as Request & { user: { id: string } }).user;
  const session =
    typeof req.query.session === "string" ? req.query.session.slice(0, 64) : null;
